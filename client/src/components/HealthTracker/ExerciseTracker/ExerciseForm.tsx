...
import { Plus, X, Activity, Flame, Clock, ChevronRight } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {Object.entries(EXERCISE_CATEGORIES).map(([key, category]) => (
          ...
          >
            <div className="text-2xl mb-2">{category.label.split(' ')[1]}</div>
            <span className="text-sm font-medium">{category.label.split(' ')[0]}</span>
          </button>
        ))}
      </div>
    </div>
