...
export function RecipeQA({ recipe }: RecipeQAProps) {
  ...
  const dragProgress = useTransform(
    dragY,
    [0, window.innerHeight * 0.1], // Reducir la distancia necesaria para ocultar
    [0, 1]
  );
  ...
  // Calcular la opacidad del header basada en el drag
  ...
  // Modificar handleSendMessage para expandir el chat
  ...
  return (
    <>
      {/* Input siempre visible y por encima */}
      <div className="fixed inset-x-0 bottom-0 px-4 py-3 bg-white/95 backdrop-blur-sm border-t border-rose-100 z-50">
        <ChatInput
          value={input}
          onChange={setInput}
      ...
      {/* Contenedor principal con z-index menor */}
      <motion.div
        initial={false}
        animate={isExpanded ? 'expanded' : (isHeaderVisible ? 'collapsed' : 'hidden')}
        variants={variants}
        drag="y"
      ...
      >
        ...
        {/* Content */}
        <div 
          className={`h-full flex flex-col bg-gradient-to-b from-violet-50/50 via-fuchsia-50/50 to-rose-50/50 transition-all duration-200 ${
            !isHeaderVisible && !isExpanded ? 'pt-0' : ''
          }`}
          onTouchStart={(e) => {
