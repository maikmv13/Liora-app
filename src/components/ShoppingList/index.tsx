...
export function ShoppingList({ items, onToggleItem }: ShoppingListProps) {
  ...
  const adjustedItems = items.map(item => ({
    ...item,
    cantidad: (item.cantidad / 2) * servings // Dividimos por 2 (comensales base) y multiplicamos por los comensales deseados
  }));
  ...
  const itemsByCategory = filteredItems.reduce((acc, item) => {
    ...
    // Si estamos en vista diaria, solo mostramos los días seleccionados
    if (viewMode === 'daily') {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">Lista de Compra</h2>
          <p className="text-gray-600 mt-1">Ingredientes necesarios para tu menú semanal</p>
        </div>
        <div className="flex space-x-4">
          <button 
            onClick={() => setShowCompleted(!showCompleted)}
            className="flex items-center space-x-2 px-4 py-2.5 bg-white/80 backdrop-blur-sm text-rose-500 rounded-xl hover:bg-white/90 transition-colors border border-rose-100 shadow-sm"
          >
            <Filter size={20} />
          ...
          <button 
            ...
            className="flex items-center space-x-2 px-4 py-2.5 bg-gradient-to-r from-orange-400 via-pink-500 to-rose-500 text-white rounded-xl hover:from-orange-500 hover:via-pink-600 hover:to-rose-600 transition-colors shadow-sm"
          >
            <Share2 size={20} />
            <span>Compartir</span>
          </button>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex gap-2">
          <div className="flex items-center space-x-2 px-4 py-2.5 bg-white/80 backdrop-blur-sm rounded-xl border border-rose-100">
